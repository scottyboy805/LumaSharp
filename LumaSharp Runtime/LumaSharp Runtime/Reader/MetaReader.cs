...
namespace LumaSharp.Runtime.Reader
{
    internal sealed class MetaReader
    {
        ...
        // Constructor
        internal MetaReader(AssemblyReader assemblyReader)
        {
        ...
        // Methods
        private void ReadAssemblyMetadata()
        {
            // Seek to offset
            assemblyReader.Stream.Seek(assemblyReader.Header.MetadataOffset, SeekOrigin.Begin);

            // Read members in order
            ReadTypeDefinitions(assemblyReader.AssemblyContext);
            ReadFieldDefinitions(assemblyReader.AssemblyContext);
            ReadMethodDefinitions(assemblyReader.AssemblyContext);
        }
